...
export default function UploadPage() {
  ...
  function handleDrop(e: React.DragEvent) {
    ...
    if (dropped?.name.endsWith('.xlsx')) {
      setFile(dropped); setPreview(null); setSaved(null); setStep('upload')
    }
  }
  ...
  function reset() {
    setFile(null); setPreview(null); setSaved(null); setStep('upload')
  }
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-8">
        ...
        <section className="bg-brand-surface border border-brand-border rounded-xl p-6">
          ...
          <p className="text-sm text-gray-400 mb-4">
            아래 템플릿을 받아서 데이터를 입력하세요. 각 시트의 예시 행을 참고해 동일한 형식으로 작성합니다.
          </p>
          <a
        ...
        {preview && step === 'preview' && (
          <section className="bg-brand-surface border border-brand-border rounded-xl p-6 space-y-6">
            ...
            <div className="flex gap-3">
              ...
              >
                {loading ? '저장 중...' : `DB에 저장 (${preview.totalRows}행)`}
              </button>
              <button
                onClick={reset}
              ...
              {Object.entries(saved.saveResults).map(([sheet, result]) => (
                <div
                  ...
                  className={cn(
                    ...
                    result.error
                      ? 'border-kpi-danger/30 bg-kpi-danger/10'
                      : 'border-kpi-success/30 bg-kpi-success/10'
                  )}
                >
